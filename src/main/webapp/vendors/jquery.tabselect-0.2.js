    ...
    $.tabSelect = function(element, options) {
        ...
        plugin.onChange = function(){
			if (plugin.settings.formElement != undefined){
				if ($(plugin.settings.formElement).is("select")){
					...
					var s=$(plugin.settings.formElement).val().split(",");
					var tmp=[];
					for (var i=0;i<s.length;i++){
						//alert(s[i]+"--"+plugin.settings.tag);
						if (s[i].indexOf(plugin.settings.tag)==-1){
							//alert("push");
							tmp.push(s[i]);
						}
					}
					$(plugin.settings.formElement).val(tmp.concat(plugin.getAllSelected()));
					//arrayObj.concat(tmp,plugin.getAllSelected())
		...
		plugin.getAllSelected = function() {
			...
			$element.find(plugin.settings.elementType).each(function(){
				if ($(this).hasClass(plugin.settings.activeClass)){
					//alert(plugin.settings.tabElements.length);
					for (var i=0;i<plugin.settings.tabElements.length;i++){
						if (plugin.settings.tabElements[i]==$(this).html()){
							values.push(plugin.settings.tabElementVals[i]);
						}
					}
		...
		plugin.selectSome=function(condition){
			var cnt = 0;
			var cc=condition.split(",");
			var tmp=[];
			for (var c in cc){
				for (var t in plugin.settings.tabElementVals){
					if (cc[c]==plugin.settings.tabElementVals[t]){
						tmp.push(plugin.settings.tabElements[t]);
						break;
					}
				}
			}
			...
			$element.find(plugin.settings.elementType).each(function(){
				for (var t in tmp)
				if ($(this).html()==tmp[t] && $(this).hasClass(plugin.settings.inactiveClass)){
					$(this).removeClass(plugin.settings.inactiveClass).addClass(plugin.settings.activeClass);
					cnt++;
				}
			});
