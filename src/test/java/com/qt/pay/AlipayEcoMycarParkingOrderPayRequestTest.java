package com.qt.pay;

import java.util.Random;

import com.alibaba.fastjson.JSONObject;
import com.alipay.api.AlipayApiException;
import com.alipay.api.AlipayClient;
import com.alipay.api.DefaultAlipayClient;
import com.alipay.api.request.AlipayEcoMycarParkingOrderPayRequest;
import com.alipay.api.response.AlipayEcoMycarParkingOrderPayResponse;
import com.qt.sales.utils.DateUtil;

/**
 * 停车费代扣
 * @author 张英辉
 *
 */
public class AlipayEcoMycarParkingOrderPayRequestTest {


    static String APP_ID = "2016080700191244";
    static String APP_PRIVATE_KEY = "";
    static String ALIPAY_PUBLIC_KEY = "";
    static String sign_type = "RSA2";
    
    // 编码格式
    static String charset = "UTF-8";
    // 支付宝沙箱网关
    static String gatewayUrl = "https://openapi.alipaydev.com/gateway.do";
    public static void main(String[] args){
    	try {
			AlipayClient alipayClient = new DefaultAlipayClient(gatewayUrl, APP_ID, APP_PRIVATE_KEY, "json", charset, 
			        ALIPAY_PUBLIC_KEY, sign_type);
			
			AlipayEcoMycarParkingOrderPayRequest request = new AlipayEcoMycarParkingOrderPayRequest();
//			request.putOtherTextParam("app_auth_token", "201710BBb26773c515324b6fb642004cab7d1X63");
			request.setBizContent(getOrderPayBiz());
			AlipayEcoMycarParkingOrderPayResponse response = alipayClient.execute(request);
			if(response.isSuccess()){
			System.out.println("调用成功");
			} else {
			System.out.println("调用失败");
			}
		} catch (AlipayApiException e) {
			e.printStackTrace();
		}
    }
    
    public static Random random = new Random();
    public static String getOrderPayBiz(){
    	JSONObject data = new JSONObject();
    	data.put("car_number", "沪C8H9K3");
    	data.put("out_trade_no", DateUtil.getCurrDateAndTime());
    	data.put("subject", "上海张江停车场代扣缴费");
    	data.put("total_fee", "15.50");
//    	data.put("seller_logon_id", "qcxdac2965@sandbox.com");
    	data.put("seller_id", "2088102170404632");
    	data.put("parking_id", "PI1509859569317717226");
//    	data.put("out_parking_id", "10066");
//    	data.put("agent_id", "2088102170404632");
    	data.put("car_number_color", "蓝色汽车");
    	String jsonStr =  data.toJSONString();
    	System.out.println(jsonStr);
    	return jsonStr;
    }
	
	
}
