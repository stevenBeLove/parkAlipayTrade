...
public class PersonalCommissionSqlProvider {
    ...
    public static String PersonalCommissionCount(PersonalCommissionVO personalCommissionVO) {
        ...
        sb.append("  from (select agency_id,customerid,localdate,(sum(parent_profit) + sum(two_profit) +sum(top_profit))  month_amount from (select agency_id,  parent_customerid customerid, substr(localdate, 1, 6) localdate,sum(parent_profit) parent_profit, 0 two_profit, 0 top_profit from qtfans.tb_fans_customer_jnls group by agency_id,  parent_customerid,  substr(localdate, 1, 6) union all  select agency_id,  two_customerid customerid,  substr(localdate, 1, 6) localdate,   0 parent_profit,   sum(two_profit) two_profit,  0 top_profit  from qtfans.tb_fans_customer_jnls  group by agency_id,       two_customerid,    substr(localdate, 1, 6) union all select agency_id,  top_customerid customerid,  substr(localdate, 1, 6) localdate,  0 parent_profit,  0 two_profit,  sum(top_profit) top_profit  from qtfans.tb_fans_customer_jnls  group by agency_id,   top_customerid,   substr(localdate, 1, 6))   group by agency_id, customerid, localdate) t1 inner join qtpay.paycustomer t2  on t1.customerid = t2.customerid  inner join qtfans.tb_fans_customer t3  on t1.customerid = t3.customerid inner join (select fs_usercode, substr(fs_origdate, 1, 6) month_date,  sum(FD_AMT) FD_AMT,  sum(FD_REALFEE) FD_REALFEE,    max(fs_origdate) fs_origdate   from qtfund.tdflist   where fs_inchannel in ('20','21')   group by fs_usercode, substr(fs_origdate, 1, 6)) t4   on t1.customerid = t4.fs_usercode inner join (select agency_id, agency_name from TB_FANS_AGENCY) ay on t1.agency_id = ay.agency_id and t1.localdate = t4.month_date");
